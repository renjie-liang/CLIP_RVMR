...
from modules.ReLoCLNet_lib.model_components import BertAttention, LinearLayer, BertSelfAttention, TrainablePositionalEncoding
from modules.ReLoCLNet_lib.model_components import MILNCELoss
from modules.ReLoCLNet_lib.contrastive import batch_video_query_loss
...
class ReLoCLNet(nn.Module):
    def __init__(self, config):
        ...
        self.query_pos_embed = TrainablePositionalEncoding(max_position_embeddings=config.max_qlen,
                                                           hidden_size=config.hidden_size, dropout=config.input_drop)
        self.ctx_pos_embed = TrainablePositionalEncoding(max_position_embeddings=config.max_vlen,
                                                         hidden_size=config.hidden_size, dropout=config.input_drop)
    ...
    def infer_onevideo(self, query_feat, query_mask, video_feat, video_mask, sub_feat, sub_mask):
        video_feat, sub_feat, mid_x_video_feat, mid_x_sub_feat, x_video_feat, x_sub_feat = self.encode_context(
            video_feat, video_mask, sub_feat, sub_mask, return_mid_output=True)
        video_query, sub_query, query_context_scores, st_prob, ed_prob = self.get_pred_from_raw_query(
            query_feat, query_mask, x_video_feat, video_mask, x_sub_feat, sub_mask, cross=False,
